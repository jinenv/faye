...
async def populate_static_data():
    ...
    # Corrected: Use the full relative path for get_config
    ...
    async with AsyncSessionLocal() as session:
        ...
        if count == 0:
            ...
            # Ensure esprits_data_from_json is a dictionary before iterating
            if esprits_data_from_json and isinstance(esprits_data_from_json, dict):
                ...
                log.warning("Esprits data from JSON is not a valid dictionary or is empty. Skipping population.")
        else:
